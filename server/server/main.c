...
void GestionarInvitaciones(ListaPartidas *listaPartidas, Partida *partida, ConnectedList *list) {
    ...
    sum = 0;
    for (int i = 0; i < 4; i++) {
        if (partida->answer[i] == -1) { // Invitation denied if any answer is -1
            denegado = 1;
            break;
        }
...
int GestionarCrearPartida(int pos, ConnectedList *list, ListaPartidas *listaPartidas, Nombre name1, Nombre name2,
    ...
    for (int i = 1; i < sizeof(is) / sizeof(is[0]); i++) {
        ...
        pthread_mutex_lock(&invitation_mutex); // Lock the mutex before modifying the invitation status
        ...
        pthread_mutex_unlock(&invitation_mutex); // Unlock the mutex after modifying the invitation status
    }
...
void *AtenderThread(ThreadArgs *threadArgs) {
    ...
    while (vacios < 6) {
        ...
        switch (code) {
            ...
            case 9:
                logger(LOGINFO, "Se ha recibido un mensage en el chat"); // Log a message indicating that a chat message has been received
                p = strtok(NULL, "/");
        ...
        if (send_awr == 1) {
            if (list->connections[pos].sending_connected == 1) {
                ...
                logger(LOGINFO, logmsg); // Log a message indicating that the connection is waiting for the list of connected clients to be sent
    ...
    pthread_mutex_unlock(&main_mutex); // Acquire the main mutex to avoid interruption
    pthread_mutex_lock(&glubal_update_mutex); // Acquire the global update mutex to avoid interruption
...
int main() {
    ...
    if (strcmp(buf, "antonia") == 0) {
        logger(LOGINFO, "Se ha detectado como maquina virtual"); // Log a message indicating that the server has been detected as a virtual machine
        snprintf(DBSERVER, 30, LDBSERVER); // Set the database server address to the virtual machine address
    } else {
        logger(LOGINFO, "Se ha detectado como servidor de producción"); // Log a message indicating that the server has been detected as a production server
        snprintf(DBSERVER, 30, PDBSERVER);// Set the database server address to the production server address
    }
    ...
    ConnectedList *list = (ConnectedList *) malloc(sizeof(ConnectedList));// Allocate memory for the connected client list
    ...
    pthread_create(&update_thread, NULL, (void *(*)(void *)) UpdateThreads, list); // Create a thread for updating the client connections
    int i = 0;
